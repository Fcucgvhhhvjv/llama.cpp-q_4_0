...
static bool bonsai_eval_internal(
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        // self-attention
        {
            ...
            // Q = Qcur.contiguous().view(head_dim, n_head, N).permute(0, 2, 1, 3) // [ batch, n_head, head_dim, N ]
            struct ggml_tensor * Q =
                ggml_permute(ctx,
                        Qcur,
                        0, 2, 1, 3);

            // K = Kmem.view(head_dim, n_head, n_past + N).permute(0, 2, 1, 3) // [ batch, n_head, head_dim, N ]
            struct ggml_tensor * K =
