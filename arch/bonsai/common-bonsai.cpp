...
bool gpt_params_parse(int argc, char ** argv, gpt_params & params) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-s" || arg == "--seed") {
            ...
            params.logit_bias[bonsai_token_eos()] = -INFINITY;
...
void gpt_print_usage(int /*argc*/, char ** argv, const gpt_params & params) {
    ...
    if (bonsai_mlock_supported()) {
        fprintf(stderr, "  --mlock               force system to keep model in RAM rather than swapping or compressing\n");
    }
    if (bonsai_mmap_supported()) {
...
std::vector<bonsai_token> bonsai_tokenize(struct bonsai_context * ctx, const std::string & text, bool add_bos) {
    // initialize to prompt numer of chars, since n_tokens <= n_prompt_chars
    std::vector<bonsai_token> res(text.size() + (int)add_bos);
    int n = bonsai_tokenize(ctx, text.c_str(), res.data(), res.size(), add_bos);
    assert(n >= 0);
    res.resize(n);
